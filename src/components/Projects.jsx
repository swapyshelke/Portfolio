import React from "react";
import { TbExternalLink } from "react-icons/tb";
import { motion } from "framer-motion";
import useFetchData from "../Hooks/useFetchData";

// const projects = [
//   {
//     id: 1,
//     title: "Real Time Code Editor",
//     description:
//       "A powerfull tool for coding with your friends, in real time. Build using Websockets.",
//     image: "/assets/project1.png",
//     link: "https://real-time-code-editor-swapnilshelke.netlify.app",
//   },
//   {
//     id: 2,
//     title: "Ambient Audio Mixer",
//     description:
//       "Platform where users can listen and mix all kinds of ambient music. \n Build in Vanilla JS, HMTL and CSS",
//     image: "/assets/project2.png",
//     link: "https://ambient-swapnilshelke.netlify.app",
//   },
//   {
//     id: 3,
//     title: "Expense Tracker",
//     description: "React Mini Project, where user can track their expenses \n ",
//     image: "/assets/project3.png",
//     link: "https://expensetracker-swapyshelke.netlify.app",
//   },
//   {
//     id: 4,
//     title: "Quick Notes Taking App",
//     description: "This is Vanilla JavaScript App. \n ",
//     image: "/assets/project4.png",
//     link: "https://quick-notes-swapyshelke.netlify.app",
//   },
//   {
//     id: 5,
//     title: "Todo App - Foucus on Today",
//     description: "This is Vanilla JavaScript App. \n ",
//     image: "/assets/project5.png",
//     link: "https://modern-todos-swapnilshelke.netlify.app",
//   },
// ];


export default function Projects() {

  const { data, loading, error } = useFetchData(
    "https://api.github.com/users/swapyshelke/repos"
  );

  
  if (loading) return <div>Loading repositories...</div>;
  if (error) return <div>Error fetching data: {error.message}</div>;

  // fetching data from github
  return (
    <>
      <h1>My Projects</h1>
      <ul>
        {data &&
          data.map((repo) => (
            <li key={repo.id}>
              <a
                href={repo.html_url}
                target="_blank"
                rel="noopener noreference"
              >
                {repo.name}
              </a>{" "}
              - {repo.description || "No Description Yet"}
            </li>
          ))}
      </ul>

      {/* // Original component /* return (
      <div
        className="bg-black px-5 lg:px-28 py-8 my-8 lg:py-16 lg:my-16"
        id="projects"
      >
        <h2 className="text-2xl lg:text-4xl text-center text-white">
          My <span className="font-extrabold">Projects</span>
        </h2>

        <div className="lg:mt-16 mt-8 lg:space-y-16 space-y-8 lg:pb-6 pb-3">
          {projects.map((project, index) => (
            <motion.div
              key={project.id}
              className={`flex justify-between items-center flex-col gap-3 ${
                index % 2 === 0 ? "lg:flex-row" : "lg:flex-row-reverse"
              }`}
              initial={{ opacity: 0, y: 50 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{
                type: "spring",
                stiffness: 80,
                damping: 10,
                delay: index * 0.2,
              }}
              viewport={{ once: true }}
            >
              <div className="lg:w-[500px] w-full rounded-2xl overflow-hidden">
                <img
                  className="w-full h-full hover:scale-105 transition-all duration-500 cursor-pointer object-cover"
                  src={project.image}
                  alt={project.title}
                />
              </div>

              <div className="lg:w-1/2 lg:space-y-6 space-y-4">
                <h2 className="font-extrabold text-white mt-5 lg:mt-0 text-3xl lg:text-5xl">
                  {String(project.id).padStart(2, "0")}
                </h2>
                <p className="font-bold text-white text-xl lg:text-3xl">
                  {project.title}
                </p>

                <p className="font-light text-sm/6 lg:text-base text-[#71717A]">
                  {project.description}
                </p>
                <a
                  href={project.link}
                  className="text-white mt-3 block"
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  <TbExternalLink size={23} />
                </a>
              </div>
            </motion.div>
          ))}
        </div>
      </div>
      );  */}
    </>
  );
}
